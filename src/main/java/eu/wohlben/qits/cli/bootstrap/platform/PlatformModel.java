...
public final class PlatformModel {
     ...
     * a cd deployment is skipped at compose-up, and the deploy loop hands the rest over.
    ...
    public static final List<String> CORE =
            List.of("gateway", "artifacts", "ci", "cd", "idp", "serviceregistry");
     ...
     * its own cutover must not fall inside another application's deploy window), serviceregistry
     * after it (cd proxies every environment operation to the registry), the seed's own repos
     * last, cd at the very end — its deployment is the self-update handoff.
     */
    public static final List<String> DEPLOYABLES = List.of(
            "observability", "idp", "serviceregistry", "stt", "projects", "workspaces", "events",
            "gateway", "artifacts", "ci", "cd");

    /**
     * The deployables that are NOT environment applications: platform-plane singletons, one
     * instance for the whole platform, deployed from main rather than from the environment's
     * branch. The authority is each repo's {@code .config/qits/deployments.yml}; this list is what
     * tells the bootstrap which ref deploys them and which container name to expect.
     */
    public static final List<String> SINGLETONS = List.of("idp", "serviceregistry");

    /**
     * Repositories that participate in release trains but are not applications of qits-cd. They
     * still need a repository on the platform git host and a main push: qits-projects can then
     * inventory them, qits-workspaces can release them, and qits-ci can discover their event
     * pipelines.
     * <p>
     ...
     */
    public static final List<String> RELEASE_TRAIN_REPOS = List.of(
    ...
    /** Every pipeline repository: the deployables and the release trains. */
    public static List<String> platformRepos() {
        List<String> all = new ArrayList<>(DEPLOYABLES);
        all.addAll(RELEASE_TRAIN_REPOS);
    ...
    public static boolean isSingleton(String name) {
        return SINGLETONS.contains(name);
    }

    /**
     * What cd names the container it manages for this application. Two shapes because the model
     * has two: environment applications carry the environment's name, singletons carry
     * 'singleton'.
     */
    public static String cdNamePrefix(String name, String envName) {
        return isSingleton(name)
                ? "qits-cd-singleton-qits-" + name + "-"
                : "qits-cd-" + envName + "-qits-" + name + "-";
    }

    /** The ref whose green build deploys this application. The other ref is pushed too, quietly. */
    public static String deployRef(String name, String envBranch) {
        return isSingleton(name) ? "main" : envBranch;
    }
