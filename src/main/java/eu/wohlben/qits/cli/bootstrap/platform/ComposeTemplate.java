...
/**
 * The seed stack and cd's run-args, generated. Both are ports of the heredocs in
...
public final class ComposeTemplate {
    ...
    private static final String COMPOSE = """
            ...
            services:
              ...
              # Ready at /serviceregistry/q/health/ready.
              ...
              qits-artifacts:
                ...
                ports:
                  ...
                  # default insecure list, so plain HTTP needs no daemon config. It is also this CLI's own
                  # door: the bootstrap runs on the host now, not as a container on qits-net.
              ...
              qits-ci:
                ...
                environment:
                  ...
                  # claim covers the event's repo. Outbound: the build-succeeded notify to qits-cd carries one.
                  QITS_AUTH_MACHINE_REQUIRED: "${MACHINE_REQUIRED}"
                  QUARKUS_OIDC_AUTH_SERVER_URL: ${IDP}
                  QUARKUS_OIDC_CLIENT_CLIENT_ENABLED: "${MACHINE_CLIENT}"
                  QUARKUS_OIDC_CLIENT_AUTH_SERVER_URL: ${IDP}
              ...
              qits-cd:
                ...
                environment:
                  ...
                  # written by the bootstrap), NOT here: a self-update's successor must inherit them, and env
                  # cannot nest those values.
                  # Machine auth, both directions. Inbound: /cd/api/events/build-succeeded demands a bearer
    ...
    private static final String RUN_ARGS = """
            # Generated by qits-cli-bootstrap — qits.cd.run-args.<application>: what each deployed container
            ...
            # cd is a SENDER: the environment endpoints it still serves write through to
            ...
            qits.cd.run-args.qits-workspaces=-v qits-workspaces-data:/data -v /var/run/docker.sock:/var/run/docker.sock --group-add ${DOCKER_GID} -e QUARKUS_DATASOURCE_WORKSPACES_JDBC_URL=jdbc:h2:file:/data/workspaces/h2/workspaces -e QUARKUS_DATASOURCE_EVENTSTREAM_JDBC_URL=jdbc:h2:file:/data/eventstream/h2/eventstream -e QITS_PROJECTS_URL=http://qits-projects:8080 -e QITS_ARTIFACTS_URL=http://qits-artifacts:8080 -e QITS_EVENTS_URL=http://qits-events:8080 -e QITS_WORKSPACE_GIT_HOST=qits-gateway
