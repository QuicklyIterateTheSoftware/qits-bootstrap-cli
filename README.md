...
for 50. `QITS_DOMAIN` adds two more, marked below.
...
| 37 | the ci-daemon musl static binary, and its digest |
| 38 | resolve the idp's client secrets (given, kept, generated) and record the run state |
| 39–40 | generate the seed stack file; write the deployer's per-application extras onto its config volume. That file holds EXTRAS and nothing else — what the deployer configures itself with is `QITS_PLATFORM_DEPLOYMENTS_*` env, spelled on the seed service and on the deployer's own extras block alike |
...
Phase 39 restarts the seed deployer when the extras it just wrote differ from what the volume
