...
Built from configuration at startup, so the count in the header is real. A cold boot is 72 phases;
...
| 43–45 | create the 40 repositories on qits-githost; push the seeded repositories to it; pre-seed the seeded histories with `-o qits.no-ci`. The seeded push is here rather than at the end because every deployable's gitlinks have to be advertised before ci clones it |
