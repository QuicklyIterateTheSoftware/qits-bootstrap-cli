...
| 48–54 | replay each publisher the platform pins by **pushing its release tag**, and wait for the run that push starts. **The recipes moved out from under this phase on 2026-09-04 and it has not caught up**: every one of the seven now declares `event: SCMRelease` in its `ci-event-release.yml`, where it declared `SCMPublishTag` when this phase was written. A pushed tag still announces `SCMPublishTag` and nothing selects it any more, so the tag alone starts no run and this phase waits its budget out. It announces no `SCMRelease` by hand on purpose — that word means "a version is NEW", and saying it here woke the release train against a platform whose consumers are still fifteen phases away — so closing the gap is a change to this phase rather than a line to add here. Four of the seven are the Maven and npm packages the wrapper's builds install; three are docker images — `qits/workspace-base`, then `qits/workspace` and `qits/projects-daemon` + `qits/project-agent`. **The base goes first and that order is load-bearing**: both daemon builds pull it at a pinned version, and the base's own replay is what puts it in the registry. A publisher with no release tag reachable from main STOPS the boot, which is right: a pin nobody has minted has nothing to dangle. A tag the git host already has moves no ref, so it announces nothing and the phase is SKIPPED — the registry holds that version from the boot that first pushed it. **The newest release is not the only version replayed**: consumer poms lag, so every version of the publisher `PinnedVersions` still finds pinned is pushed too, oldest first and the newest last, each on exactly those terms — except for a publisher the seed already publishes as a library (phases 8 and 18–25 put every pinned version of those five in the store, so replaying their older tags would be a ci run per tag for bytes the registry has) |
