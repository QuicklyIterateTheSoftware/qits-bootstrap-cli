...
| 40 | resolve the idp's client secrets (given, kept, generated) and record the run state |
| 41–42 | generate the seed stack file; write the deployer's per-application extras onto its config volume. That file holds EXTRAS and nothing else — what the deployer configures itself with is `QITS_PLATFORM_DEPLOYMENTS_*` env, spelled on the seed service and on the deployer's own extras block alike |
