...
| `QITS_SHIP_MAINS` | `0` | **ignored**, and kept only so an old `.env` does not fail a boot. It built and deployed the local mains instead of RESTORING each deployable's and publisher's newest release tag. A deployment is `qits/<app>:<version>` now and an unreleased main has no version, so there is nothing to point at — and honouring the build half alone would seed from main beside deployments of the last release, which is the Flyway mismatch the restore exists to prevent. A run that sets it is warned in the `sources` phase. Deploying your commit means cutting a release request in qits-projects |
...
| `QITS_HOST_OOM` | `1` | 0 = leave this host's answer to memory pressure alone. On, the `host-oom` phase installs `systemd-oomd`, writes its two slice rules and the omit list, and makes an 8G `/swapfile` where the host has neither swap nor one already — so a squeeze kills one workload instead of hanging the machine. Off is for a node whose memory policy somebody else owns |
...
| `QITS_ENV_NAME` | `prod` | the environment. It is the **platform environment** — the one tier every release enters at, and the one the platform plane is deployed into — and it is inside every wire alias, every deployed container name and every idp client id. There is no deploy ref beside it. `--platform-env` is the same knob for one run |
| `QITS_IDP_CLIENT_<ID>_SECRET` | generated | pin the BOOTSTRAP's own idp client secret instead of generating it — `<ID>` is `<env>-qits-bootstrap`, the one credential this program still holds. Every other client is created against the running idp and its secret is kept in qits-deployments' registry, so there is nothing here to pin |
...
**The second is the `ipv6-loopback` phase's**, and the boot installs it rather than asking for it —
whoever started the run. A `*.localhost` name resolves to `::1` first, and swarm's ingress mesh is
IPv4-only: the listener ACCEPTS the v6 connection and never serves it, so curl, docker, git, maven
and npm all hang rather than fail over — a release run's SBOM upload sat there forty minutes on
2026-09-02 and blocked the CI queue behind it, and a CI step's `docker push` timed out the same way
on a cold node on 2026-09-05. The reset makes each client fall back at once, where a DROP would leave
them hanging exactly as the accept does. A host-mode publish never showed this — docker-proxy bound
both families — so it arrives with ingress.

The rule does **not** survive a reboot, which is why every run installs it rather than trusting the
last one's, and it is installed **unconditionally**: the phase runs in the plan's first minutes and
the ingress does not bind that port until the seed stack is up, so a probe-and-repair would find
nothing to do and leave the landmine armed for the run that needs it. `-C` before `-I` is what keeps
a rerun from stacking duplicates. The payload is a container, so it acts on the host through the
daemon — a throwaway `--privileged --pid=host --network host` helper running the host's own
`ip6tables` under `nsenter -t 1 -m -n`. A helper that cannot run STOPS the boot with that one
command, because the alternative is finding out an hour later inside a CI step.

**The host's answer to memory pressure is the `host-oom` phase's, and it is a THIRD host setting
this program owns.** A fresh host has no early OOM killer and no swap, and under pressure it does
not lose a workload — it livelocks. On 2026-09-11 a 16 GB node answered ping and nothing else: ssh
and the provider's console both timed out and the machine needed a hard reset, and the kernel's own
OOM killer never fired, because with swap the host crawls to a halt before an allocation fails.
`oom_score_adj` only orders the kernel's victims once it acts, so it did not help and could not. A
failed CI run or a killed agent is always better than a host restart, and this platform builds, runs
agents and runs workspaces on the machine it serves from.

So the phase writes, through the same privileged `nsenter` helper the ipv6 rule uses:

    apt-get install -y systemd-oomd              # where the unit is missing and the host has apt
    /etc/systemd/system/system.slice.d/50-qits-oomd.conf   ManagedOOMMemoryPressure=kill, 50%
    /etc/systemd/system/-.slice.d/50-qits-oomd.conf        ManagedOOMSwap=kill
    /etc/systemd/system/<unit>.d/50-qits-oomd.conf         ManagedOOMPreference=omit
    8G /swapfile + /etc/fstab, and vm.swappiness=20        where this host has no swap at all

`systemd-oomd` reads pressure stall information per cgroup and kills a LEAF cgroup before the host
stops answering. Docker uses the systemd cgroup driver here, so every container is
`system.slice/docker-<id>.scope` and every build is a sub-cgroup of buildkitd's — which is why the
pressure rule kills the build that grew rather than the daemon that started it. The dwell is oomd's
own `DefaultMemoryPressureDurationSec=20s`. The omit list is `docker`, `containerd`, `ssh` and
`systemd-journald`, and **containerd is the load-bearing one**: every container's shim lives in its
cgroup, so oomd killing it would stop every container on the host at once. `ManagedOOMPreference` is
the only priority knob oomd has — it ignores `oom_score_adj` — and it cannot be set durably on a
docker scope, because those come and go with their containers. A `daemon-reload` is enough to put
`user.oomd_omit` on the running units' cgroups, so neither docker nor containerd is restarted for
any of this.

Everything is written only where it differs, so a rerun is quiet and the service is restarted only
when a drop-in actually changed. **Swap of any kind is enough swap**: an existing one is somebody's
decision about this machine and is left alone, an existing `/swapfile` that is off is left alone
too, and a swapfile is made only where the root filesystem has its size and 8 GB more to spare.
`vm.swappiness` is APPENDED to `/etc/sysctl.d/99-qits.conf` and never written over it — that file is
the host's. Unlike the ipv6 rule, a gap here **warns and the boot goes on**: the platform comes up
without an early killer, it just has a worse hour ahead of it, so a host with no systemd is skipped
and a host where oomd will not start gets the gap named and the commands in the closing report.
`oomctl` lists the two rules on a host that has them. `QITS_HOST_OOM=0` turns the phase off.
...
| 38 | **seed the two toolchain tarballs the ci-daemon's builder image is built from** — GraalVM's musl toolchain and the zlib source — into the store at the coordinates that image's Dockerfile reads (`eu.wohlben.qits.toolchain`). They stopped being fetched from the internet per build on 2026-09-05, which is right for a platform that has a store and a hole in a bootstrap: nothing in the estate uploads them, so a fresh host's store has neither and the image's `ADD` fails. Each is downloaded from the upstream address that Dockerfile records, checked against the sha256 it records beside it — the pins belong to the repository that consumes the bytes, and this reads them rather than keeping a second copy — and published with `deploy:deploy-file` into BOTH registries, the temporary file one first: the bootstrap ingress proxies its Maven route to that temporary registry for the whole run and is never repointed, so a tarball that reached only the store is a 404 to the `ADD` in the row below. Each gets the pom and the checksums a hand upload leaves, and whatever a registry already holds is skipped |
| 39 | the ci-daemon musl static binary, and its digest. Its builder image is told THIS run's address for the two tarballs above — the Dockerfile's own default names `registry.dev.localhost`, an edge vhost, and no edge exists this early in a boot. BuildKit resolves an `ADD` itself, outside any step, so the bootstrap capability travels in that url rather than as the build secret every `RUN` gets |
| 40 | resolve the BOOTSTRAP's own idp client (given, kept, generated) and record it in `.qits-bootstrap.env`. One pair, and the run creates every other credential with it |
| 40a | start **qits-platform-idp alone** from the seed stack file. A client cannot be created at a service that is not up, and the idp seeds its first database service client from the pair above — `QITS_IDP_SEED_CLIENT_ID`/`_SECRET`, once per installation, gated by an `idp_seed` marker row rather than by the variables, so every rerun spells them and seeds nothing twice |
| 40b | create the FIVE seed services' idp clients against that running idp (`POST /idp/api/service-clients`): qits-projects, qits-ci, qits-containers, qits-deployments and qits-platform-edge — the applications that are up before anything exists that could provision a credential for them. Each secret is recorded in qits-deployments' `pd_resource` registry, keyed by the application and this tier, which is the same row the deployer injects as `QITS_RESOURCE_IDP_*` when it replaces the seed container. Every other application declares an `idp:client` resource and is handed one by the deployer in the ordinary way |
| 41–42 | generate the seed stack file; write the deployer's per-application extras onto its config volume. **After the three phases above, because the values are not known until the idp has issued them**: each seed service's block carries `QITS_RESOURCE_IDP_URL`, `_CLIENT_ID` (the wire alias — a client id IS the alias) and `_CLIENT_SECRET`. That file holds EXTRAS and nothing else — what the deployer configures itself with is `QITS_PLATFORM_DEPLOYMENTS_*` env, spelled on the seed service and on the deployer's own extras block alike — and **nothing identity-shaped is in it at all**: no `QITS_RESOURCE_IDP_*`, no `QUARKUS_OIDC_CLIENT_*`, no `QITS_IDP_CLIENT*`. For a deployed application the deployer injects the credential from its own registry, and a stored one would shadow the row it keeps current and outlive every rotation of it |
...
queue a build of a commit nothing deploys; it pushes the newest release tag quietly too, which
...
Phase 41 restarts the seed deployer when the extras it just wrote differ from what the volume
...
## Two planes, no branch

`main` is the integration trunk of every repository and deploys nothing. **Nothing that is pushed
deploys.** What deploys is a RELEASE: qits-projects tags it, the repository's own release recipe
builds that tag and publishes `qits/<app>:<version>`, qits-ci announces one `SoftwareRelease` per
published artifact, and qits-deployments turns the docker one into a deployment request and pulls
that image. `environment/<name>` and `platform/main` are both retired, and the deployer's
environments have no branch column to hold either.

A bootstrap therefore restores a VERSION rather than a ref: per deployable it re-establishes main
and the **newest release tag**, and asks qits-ci for that release's build. So a boot is a RESTORE of
the last released platform rather than a shipment of whatever your checkouts hold — always, now:
`--ship-mains` is ignored, because an unreleased main has no version and a version is the only
coordinate the deployer takes.
...
deploy ref (retired since), six renamed repositories, a platform plane of four, and qits-platform-edge in front of
